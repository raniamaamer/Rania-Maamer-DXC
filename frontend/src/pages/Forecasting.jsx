...
/* ══ Client-side forecasting engine ═════════════════════════════════
   Trend + weekly seasonality decomposition (Prophet-inspired)
   Steps:
   1. Compute linear trend on log(y+1)
   2. Compute weekly seasonal factors (day-of-week averages)
   3. Project forward, multiply trend × seasonal
   4. Confidence intervals via residual std dev
*/
function runForecast(queue, horizonDays) {
  const data = REAL_DATA[queue]
  if (!data) return null

  // Build sorted series (workdays only, no zeros)
  const series = data.dates
    .map((d, i) => ({ ds: d, y: data.offered[i] }))
    .filter(p => p.y > 0)
  ...
  const t0 = new Date(series[0].ds).getTime()
  const tScale = 86400000

  // Log transform
  const logY = series.map(p => Math.log(p.y + 1))

  // Linear regression on log(y) ~ t
  const xs = series.map(p => (new Date(p.ds).getTime() - t0) / tScale)
  const xMean = xs.reduce((a, b) => a + b, 0) / n
  const yMean = logY.reduce((a, b) => a + b, 0) / n
  const num = xs.reduce((s, x, i) => s + (x - xMean) * (logY[i] - yMean), 0)
  const den = xs.reduce((s, x) => s + (x - xMean) ** 2, 0)
  const slope = den > 0 ? num / den : 0
  const intercept = yMean - slope * xMean

  // Weekly seasonal factors (0=Sun..6=Sat)
  const dowSums = Array(7).fill(0)
  const dowCounts = Array(7).fill(0)
  series.forEach(p => {
    const dow = new Date(p.ds).getDay()
    const trendVal = intercept + slope * ((new Date(p.ds).getTime() - t0) / tScale)
    const residual = Math.log(p.y + 1) - trendVal
    dowSums[dow] += residual
    dowCounts[dow] += 1
  })
  const dowFactor = dowSums.map((s, i) => dowCounts[i] > 0 ? s / dowCounts[i] : 0)

  // Residual std dev for confidence interval
  const residuals = series.map(p => {
    const t = (new Date(p.ds).getTime() - t0) / tScale
    const dow = new Date(p.ds).getDay()
    const pred = intercept + slope * t + dowFactor[dow]
    return Math.log(p.y + 1) - pred
  })
  const resMean = residuals.reduce((a, b) => a + b, 0) / residuals.length
  const resStd = Math.sqrt(residuals.reduce((s, r) => s + (r - resMean) ** 2, 0) / residuals.length)
  const ci80 = 1.28 * resStd  // 80% confidence interval
  ...
  for (let i = 1; i <= horizonDays; i++) {
    ...
    const t = (d.getTime() - t0) / tScale
    const dow = d.getDay()
    const logPred = intercept + slope * t + dowFactor[dow]
    const predicted = Math.max(0, Math.round(Math.exp(logPred) - 1))
    const lower = Math.max(0, Math.round(Math.exp(logPred - ci80) - 1))
    const upper = Math.max(0, Math.round(Math.exp(logPred + ci80) - 1))
    forecast.push({
      date: dateStr,
      predicted,
      lower,
  ...
  // In-sample metrics (last 30 points)
  const last30 = series.slice(-30)
  let maeSum = 0, mapeSum = 0
  last30.forEach(p => {
    const t = (new Date(p.ds).getTime() - t0) / tScale
    const dow = new Date(p.ds).getDay()
    const logPred = intercept + slope * t + dowFactor[dow]
    const pred = Math.max(0, Math.exp(logPred) - 1)
    maeSum += Math.abs(p.y - pred)
    mapeSum += Math.abs((p.y - pred) / (p.y + 1e-9))
  })
  const mae = Math.round(maeSum / last30.length * 10) / 10
  const mape = Math.round(mapeSum / last30.length * 100 * 10) / 10

  // History points (last 60)
  const history = series.slice(-60).map(p => ({ date: p.ds, actual: p.y }))

  return { forecast, history, metrics: { mae, mape } }
}
...
function ForecastChart({ history, forecast, color, horizon }) {
  ...
  return (
    <div style={{ background: DXC.bg, borderRadius: 12, padding: '20px', border: `1px solid ${DXC.border}`, minWidth: 0 }}>
...
function ForecastCalendar({ forecast }) {
  ...
  const maxVal = workdays.length ? Math.max(...workdays.map(f => f.predicted)) : 50
  const q1 = maxVal * 0.33
  const q2 = maxVal * 0.66

  function getColor(p, isWE) {
    if (isWE || p < 1) return null
    ...
    <div style={{ display: 'flex', gap: 12, marginTop: 16, flexWrap: 'wrap' }}>
      <div style={{ background: DXC.bg, border: `1px solid ${DXC.border}`, borderRadius: 12, padding: 14, width: 252, flexShrink: 0 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 2 }}>
          {DAYS_FR.map(d => <div key={d} style={{ fontSize: 9, color: DXC.textMuted, textAlign: 'center', padding: '2px 0', fontWeight: 700 }}>{d}</div>)}
          {cells.map((d, i) => {
            ...
            return (
              <div key={d} onClick={() => setSelectedWeek(weekNum)} style={{ borderRadius: 6, padding: '3px 1px', textAlign: 'center', cursor: 'pointer', minHeight: 36, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 1, background: c ? c.bg : isWE ? DXC.bgAlt : 'transparent', border: isSelected ? `1.5px solid ${DXC.blue}` : '1.5px solid transparent', opacity: isWE ? 0.7 : 1 }}>
                ...
                {fc && fc.predicted > 0 && <div style={{ fontSize: 8, color: c ? c.txt : DXC.textMuted, lineHeight: 1 }}>{fc.predicted >= 1000 ? (Math.round(fc.predicted / 100) / 10) + 'k' : fc.predicted}</div>}
              </div>
            )
          })}
        </div>
        <div style={{ display: 'flex', gap: 6, marginTop: 10, flexWrap: 'wrap' }}>
          {[['#dcfce7','#166534','Calme'],['#fef3c7','#92400e','Moyen'],['#fee2e2','#991b1b','Pic']].map(([bg,txt,label]) => (
            <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 9, color: DXC.textMuted }}>
              <div style={{ width: 9, height: 9, borderRadius: 3, background: bg, border: `1px solid ${txt}33` }} />{label}
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        <div style={{ background: DXC.bg, border: `1px solid ${DXC.border}`, borderRadius: 10, overflow: 'hidden' }}>
          <div style={{ padding: '8px 12px', borderBottom: `1px solid ${DXC.border}`, fontSize: 11, fontWeight: 700, color: DXC.textMuted, textTransform: 'uppercase' }}>Détail journalier — semaine sélectionnée</div>
          {weekDays.length === 0
...
/* ══ HorizonTab ══════════════════════════════════════════════════════ */
function HorizonTab({ forecastResult, horizon, color }) {
  ...
  if (!forecast) return (
    <div style={{ textAlign: 'center', padding: '60px 20px', color: DXC.textMuted }}>
      ...
      <div style={{ fontSize: 14, color: DXC.text, fontWeight: 700 }}>Aucune donnée de prévision</div>
    </div>
  )
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <MetricsBar forecast={forecast} metrics={metrics} />
      <ForecastChart history={history} forecast={forecast} color={color} horizon={horizon} />
      <ForecastCalendar forecast={forecast} />
      ...
      <div style={{ background: DXC.bg, borderRadius: 12, border: `1px solid ${DXC.border}`, overflow: 'hidden' }}>
        <div style={{ padding: '14px 20px', borderBottom: `1px solid ${DXC.border}`, background: DXC.bgSurface, fontSize: 12, fontWeight: 700, color: DXC.text, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
          📋 {horizon === '365d' ? 'Récapitulatif' : 'Détail journalier'}
        </div>
...
function QueueSummaryCard({ queue, selected, onClick }) {
  ...
  return (
    <div onClick={onClick} style={{ background: DXC.bg, border: `2px solid ${selected ? color : DXC.border}`, borderTop: `4px solid ${color}`, borderRadius: 14, padding: '16px 18px', cursor: 'pointer', transition: 'all .2s', boxShadow: selected ? `0 0 0 3px ${color}22, 0 4px 16px rgba(0,0,0,0.10)` : '0 1px 4px rgba(0,0,0,0.06)' }}>
      ...
      <div style={{ height: 32, marginBottom: 10, minWidth: 0 }}>
...
export default function Forecasting() {
  ...
  const HORIZON_DAYS = { '7d': 7, '30d': 30, '365d': 365 }

  // Compute forecast for selected queue + all horizons
  const forecastResults = useMemo(() => {
    if (!launched[selectedQueue]) return null
    const results = {}
    for (const [h, days] of Object.entries(HORIZON_DAYS)) {
      results[h] = runForecast(selectedQueue, days)
    }
    return results
  }, [launched, selectedQueue])

  function launchForecast() {
    setComputing(true)
    setTimeout(() => {
      setLaunched(prev => ({ ...prev, [selectedQueue]: true }))
      setComputing(false)
    }, 600)
  }

  const hasData = !!forecastResults
  const currentResult = forecastResults?.[horizon]
  ...
  return (
    <div style={{ fontFamily: "'Syne','Inter',sans-serif", background: DXC.bgSurface, minHeight: '100vh', padding: '28px 32px' }}>
      ...
      <div style={{ marginBottom: 28 }}>
        ...
        <p style={{ fontSize: 13, color: DXC.textMuted, marginTop: 4 }}>Prévision locale · tendance + saisonnalité hebdomadaire · IC 80%</p>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16, marginBottom: 28 }}>
        {queues.map(q => (
          <QueueSummaryCard key={q} queue={q} selected={selectedQueue === q} onClick={() => setSelectedQueue(q)} />
        ))}
      </div>
      ...
      <div style={{ background: DXC.bg, borderRadius: 16, border: `1px solid ${DXC.border}`, borderTop: `4px solid ${color}`, overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.07)' }}>
        ...
        <div style={{ padding: '16px 24px', borderBottom: `1px solid ${DXC.border}`, background: DXC.bgSurface, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              ...
              {hasData && forecastResults['7d']?.metrics && (
                <span style={{ fontSize: 11, color: DXC.green, marginLeft: 10, fontWeight: 700 }}>
                  ✓ MAPE {forecastResults['7d'].metrics.mape}% · MAE {forecastResults['7d'].metrics.mae}
                </span>
              )}
            </div>
          </div>
          ...
          <button
            ...
            disabled={computing}
            style={{ background: computing ? DXC.bgAlt : `linear-gradient(135deg,${color},${DXC.purple})`, color: computing ? DXC.textMuted : '#fff', border: 'none', borderRadius: 10, padding: '10px 20px', cursor: computing ? 'not-allowed' : 'pointer', fontSize: 13, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 8, fontFamily: "'Syne',sans-serif" }}
          >
            {computing
              ? <><span style={{ display: 'inline-block', animation: 'spin 0.8s linear infinite' }}>⟳</span> Calcul en cours...</>
              : hasData
              ? '🔄 Relancer'
              : '🚀 Lancer la prévision'}
          </button>
        ...
        <div style={{ padding: '24px', background: DXC.bgSurface }} className="fade-in">
          {!launched[selectedQueue] && !computing && (
            <div style={{ textAlign: 'center', padding: '60px 20px', color: DXC.textMuted }}>
              <div style={{ fontSize: 48, marginBottom: 16 }}>💡</div>
              <div style={{ fontSize: 16, fontWeight: 800, color: DXC.text, marginBottom: 8 }}>Prévision non lancée</div>
              <div style={{ fontSize: 13, maxWidth: 400, margin: '0 auto', lineHeight: 1.7 }}>
                Cliquez sur <strong>Lancer la prévision</strong> — le calcul s'effectue directement dans le navigateur à partir des données CSV.
              </div>
              <div style={{ marginTop: 20, display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap' }}>
                {['📈 Tendance linéaire log', '📅 Saisonnalité hebdomadaire', '📴 Jours fériés FR', '📉 MAE / MAPE'].map(f => (
          ...
          {computing && (
            <div style={{ textAlign: 'center', padding: '60px 20px' }}>
              ...
              <div style={{ fontSize: 16, fontWeight: 800, color: DXC.text, marginBottom: 8 }}>Calcul de la prévision...</div>
              <div style={{ fontSize: 13, color: DXC.textMuted }}>Décomposition tendance + saisonnalité + IC 80%</div>
            </div>
          )}

          {hasData && !computing && (
            <HorizonTab forecastResult={currentResult} horizon={horizon} color={color} />
          )}
        </div>
      </div>
    </div>
