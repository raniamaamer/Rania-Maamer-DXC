...
# ── Shared help-text constants (eliminates duplicate literals) ─────────────
DESK_HELP_TEXT = "Desk logique (depuis SLA.xlsx Sheet1 colonne Desk)"
HOUR_HELP_TEXT = "HH:MM format"
...
class QueueMetric(models.Model):
    ...
    hour        = models.CharField(max_length=5, db_index=True, help_text=HOUR_HELP_TEXT)
...
class HistoricalMetric(models.Model):
    ...
    start_date  = models.DateTimeField(db_index=True)
    end_date    = models.DateTimeField(null=True, blank=True)
    hour        = models.CharField(max_length=5, help_text=HOUR_HELP_TEXT)
    year        = models.IntegerField(db_index=True)
    month       = models.IntegerField(
        db_index=True,
        validators=[MinValueValidator(1), MaxValueValidator(12)],
    ...
    callback_contacts    = models.IntegerField(default=0)
    contacts_put_on_hold = models.IntegerField(
        default=0,
    ...
    handle_time       = models.FloatField(default=0.0, help_text='AHT x answered')
    total_answer_time = models.FloatField(default=0.0, help_text='ASA x answered')
    total_hold_time   = models.FloatField(
        default=0.0,
        help_text='Hold x answered (NaN→0, pas de dilution)',
    )
    answered_with_hold = models.IntegerField(
        default=0,
...
class RealtimeMetric(models.Model):
    ...
    avg_handle_time   = models.FloatField(default=0.0)
    avg_answer_time   = models.FloatField(default=0.0)
    longest_wait_time = models.FloatField(
        default=0.0,
