...
# ── Constantes help_text dupliqués ────────────────────────────────────────────
HELP_DESK = "Desk logique (depuis SLA.xlsx Sheet1 colonne Desk)"
HELP_HOUR = "HH:MM format"


class SLAConfig(models.Model):
    account          = models.CharField(max_length=100, unique=True, db_index=True)
    ans_rate_formula = models.CharField(max_length=200, blank=True, default="")
    abd_rate_formula = models.CharField(max_length=200, blank=True, default="")
    ...
    ans_sla = models.CharField(max_length=100, blank=True, default="")
    abd_sla = models.CharField(max_length=100, blank=True, default="")
...
class QueueMetric(models.Model):
    ...
    desk     = models.CharField(max_length=200, db_index=True, blank=True, default="",
                                help_text=HELP_DESK)
    account  = models.CharField(max_length=100, db_index=True, blank=True, default="")
    language = models.CharField(max_length=10,  db_index=True, blank=True, default="",
    ...
    hour        = models.CharField(max_length=5, db_index=True, help_text=HELP_HOUR)
    ...
    ans_out_sla = models.IntegerField(default=0)
    abd_out_sla = models.IntegerField(default=0)
    ...
    avg_ttc           = models.FloatField(default=0.0,
                          help_text='Avg TTC = Agent interaction time / handled (sec)')
    ...
    timeframe_ooh     = models.IntegerField(default=40,
                          help_text='Timeframe hors heures ouvrées (sec)')
    is_ooh        = models.BooleanField(default=False,
                      help_text='True si weekend ou entre 19h et 7h')
...
class HistoricalMetric(models.Model):
    ...
    start_date  = models.DateTimeField(db_index=True)
    end_date    = models.DateTimeField(null=True, blank=True)
    hour        = models.CharField(max_length=5, help_text=HELP_HOUR)
    year        = models.IntegerField(db_index=True)
    month       = models.IntegerField(db_index=True,
                    validators=[MinValueValidator(1), MaxValueValidator(12)])
    week        = models.IntegerField(
    ...
    ans_out_sla = models.IntegerField(default=0)
    abd_out_sla = models.IntegerField(default=0)
    abd_in_60   = models.IntegerField(default=0,
                    help_text="Abandons dans les 60 premières secondes")
    abd_out_60  = models.IntegerField(default=0, help_text="Abandons après 60 secondes")
    callback_contacts = models.IntegerField(default=0)
    sla_rate     = models.FloatField(default=0.0,
                    validators=[MinValueValidator(0.0), MaxValueValidator(1.0)])
    ...
    contacts_put_on_hold = models.IntegerField(default=0,
                            help_text="Number of contacts that were put on hold")
    avg_ttc           = models.FloatField(default=0.0,
                          help_text='Avg TTC = Agent interaction / handled (sec)')
    handle_time       = models.FloatField(default=0.0, help_text='AHT x answered')
    total_answer_time = models.FloatField(default=0.0, help_text='ASA x answered')
    total_hold_time   = models.FloatField(default=0.0,
                          help_text='Hold x answered (NaN→0, pas de dilution)')
    answered_with_hold = models.IntegerField(default=0,
